...
<template>
    <div class="flex h-full flex-col rounded-lg border border-gray-200 bg-white">
        ...
        <ul class="flex-1 overflow-y-auto">
            <li v-if="items.length === 0" class="p-4 text-center text-sm text-gray-400">Belum ada item</li>
            <li v-for="item in items" :key="item._id">
                <button @click="emit('select', item)" class="block w-full border-b border-gray-100 px-4 py-3 text-left transition hover:bg-brand-50" :class="{ 'bg-brand-50': selectedId === item._id }">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="rounded-full px-2 py-0.5 text-xs font-medium" :class="item.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'">
                            {{ item.status === "active" ? "Aktif" : "Nonaktif" }}
                        </span>
                    </div>
                    <div class="mt-0.5 text-xs text-gray-500">{{ item.category }} · Stok {{ item.stock }}</div>
                </button>
            </li>
        </ul>
    </div>
